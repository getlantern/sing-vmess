...
func (b *Buffer) Extend(n int) ([]byte, error) {
	...
	if end > b.capacity {
		return nil, errors.New(F.ToString("buffer overflow: capacity ", b.capacity, ",end ", b.end, ", need ", n))
	}
...
func (b *Buffer) ExtendHeader(n int) ([]byte, error) {
	if b.start < n {
		return nil, errors.New(F.ToString("buffer overflow: capacity ", b.capacity, ",start ", b.start, ", need ", n))
	}
	b.start -= n
	return b.data[b.start : b.start+n], nil
}

func (b *Buffer) WriteRandom(size int) ([]byte, error) {
	buffer, err := b.Extend(size)
	if err != nil {
		return nil, err
	}
...
func (b *Buffer) Reserve(n int) error {
	if n > b.capacity {
		return errors.New(F.ToString("buffer overflow: capacity ", b.capacity, ", need ", n))
	}
	b.capacity -= n
	return nil
}

func (b *Buffer) OverCap(n int) error {
	if b.capacity+n > len(b.data) {
		return errors.New(F.ToString("buffer overflow: capacity ", len(b.data), ", need ", b.capacity+n))
	}
	b.capacity += n
	return nil
}
...
func (b *Buffer) Release() error {
	...
	if b.refs.Load() > 0 {
		return errors.New("buffer is still in use")
	}
	err := Put(b.data)
	*b = Buffer{}
	return err
...
func (b *Buffer) Leak() error {
	if debug.Enabled {
		...
		refs := b.refs.Load()
		if refs == 0 {
			return errors.New("leaking buffer")
		} else {
			return errors.New(F.ToString("leaking buffer with ", refs, " references"))
		}
	} else {
		b.Release()
	}
